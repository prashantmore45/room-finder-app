    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 border-b border-gray-700 pb-1">
          <button onClick={() => setActiveTab('sent')} className={`pb-3 px-4 flex items-center gap-2 font-medium transition-colors ${activeTab === 'sent' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400'}`}>
            <Send size={18} /> Sent ({sentApps.length})
          </button>
          <button onClick={() => setActiveTab('received')} className={`pb-3 px-4 flex items-center gap-2 font-medium transition-colors ${activeTab === 'received' ? 'text-purple-400 border-b-2 border-purple-400' : 'text-gray-400'}`}>
            <Inbox size={18} /> Received ({receivedApps.length})
          </button>
        </div>
                 ...
                 {app.status === 'pending' && (
                   <div className="flex gap-3">
                      <button 
                        ...
                        className="flex-1 bg-green-600/20 text-green-400 hover:bg-green-600/30 py-2 rounded-lg border border-green-600/30 font-medium"
                      >
                        Accept
                      </button>
                      <button 
                        onClick={() => handleStatusUpdate(app.id, 'rejected')}
                        className="flex-1 bg-red-600/20 text-red-400 hover:bg-red-600/30 py-2 rounded-lg border border-red-600/30 font-medium"
                      >
                        Reject
                      </button>
                   </div>
